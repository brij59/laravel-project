...
{{ $configData['sidebarClass'] }} {{ $configData['footerType'] }}"
    ...
    @if (isset($configData['showMenu']) && $configData['showMenu'] === true)
        @include('user/panels.sidebar')
    @endif

    {{-- Include Navbar --}}
    @include('user/panels.navbar')
        ...
        @else
            <div class="content-wrapper {{ $configData['layoutWidth'] === 'boxed' ? 'container p-0' : '' }}">
                ...
                @if ($configData['pageHeader'] === true && isset($configData['pageHeader']))
                    @include('user/panels.breadcrumb')
                @endif
    ...
    @if ($configData['blankPage'] == false && isset($configData['blankPage']))
        @include('user/content/pages/customizer')

        @include('user/content/pages/buy-now')
    @endif
    ...
    {{-- include footer --}}
    @include('user/panels/footer')

    {{-- include default scripts --}}
    @include('user/panels/scripts')
